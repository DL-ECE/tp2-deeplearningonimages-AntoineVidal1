...
Original file is located at
    https://colab.research.google.com/drive/1xB-3ltG4oaktpXpgd5MAp71RqZaqKlAx
...
def convolution_forward_numpy(image, kernel):
    I1= np.zeros((image.shape[0]+2,image.shape[1]+2))
    I1[1:I1.shape[0]-1,1:I1.shape[1]-1] = image

    H,W = I1.shape
    M,N = kernel.shape
    K = np.zeros((H-M+1,W-N+1), dtype=float)
    
    for i in range(H-M+1):
      for j in range(W-N+1):
        K[i,j] = np.sum( I1[i:i+M,j:j+N] * kernel)
...
kernel=K_1
# YOUR CODE HERE
output_image = convolution_forward_numpy(image[:,:,0],kernel) 
...
def convolution_forward_torch(image, kernel):
    # YOUR CODE HERE 
    tensor_arr = torch.from_numpy(image)
    tensor_ker = torch.from_numpy(kernel)
    tsr_arr = torch.unsqueeze(tensor_arr, 0) ## Yes, you need two invocations of unsqueeze
    tsr_arr = torch.unsqueeze(tsr_arr, 0)
    tsr_ker = torch.unsqueeze(tensor_ker, 0) ## Yes, you need two invocations of unsqueeze
    tsr_ker = torch.unsqueeze(tsr_ker, 0)
    output = nn.functional.conv2d( tsr_arr.double(), tsr_ker.double(), stride=1,padding=1)
    #print(output)

#print(convolution_forward_torch(I,K_1))
...
class CNNModel(nn.Module):
  def __init__(self, classes=10):
        ...
        self.conv2 = torch.nn.Conv2d(64,256,kernel_size=3,padding = 1)
        
        self.conv3 = torch.nn.Conv2d(256,128,kernel_size=3,padding = 1)
        ...
        self.f1 = nn.Linear(64*7*7,150)
        self.f2 = nn.Linear(150, 80)
